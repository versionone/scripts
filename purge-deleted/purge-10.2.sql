...
--dbo.Scheme
--dbo.Scheme_Now
insert @gather_ids select ID from dbo.Scheme_Now where AssetState=255
...
--dbo.SchemeSelectedValues
delete dbo.SchemeSelectedValues where SchemeID in (select ID from @ids) or ListID in (select ID from @ids)

--dbo.AttributeDefinitionVisibility
delete dbo.AttributeDefinitionVisibility where SchemeID in (select ID from @ids)

--dbo.Scheme
--dbo.Scheme_Now
delete dbo.Scheme where ID in (select ID from @ids)
delete dbo.Scheme_Now where ID in (select ID from @ids)
...
delete @strings from (select distinct(ListValidValuesFilter) Z from dbo.AssetType) X where Z=ID
delete @strings from (select distinct(Name) Z from dbo.Scheme) X where Z=ID
