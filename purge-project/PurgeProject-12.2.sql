...
declare @scopeToPurge int; --set @scopeToPurge = 301530
declare @allowRecursion bit; --set @allowRecursion = 1

-- Ensure the correct database version
declare @supportedVersion varchar(10); set @supportedVersion = '12.2'
...
-- doom TeamRooms tied to doomed Scopes
insert @doomed
...
select @error=@@ERROR; if @error<>0 goto ERR
update TeamRoom_Now set ScheduleID=null from @doomed where doomed=ScheduleID
select @error=@@ERROR; if @error<>0 goto ERR
...
select @error=@@ERROR; if @error<>0 goto ERR
update TeamRoom set ScheduleID=null from @doomed where doomed=ScheduleID
select @error=@@ERROR; if @error<>0 goto ERR
